...
if filtered_df.empty:
    ...
    # 수정된 분석 프롬프트
    analysis_prompt = f"""
    아래에 제공된 뉴스 기사들을 분석하여, 1. 2. 3. 4.과 같이 최대 4개의 번호로 핵심 트렌드와 사견을 요약해 줘. 각 항목은 한 문장으로 작성하고, 줄바꿈으로 구분해. 특별한 서식(볼드체, 따옴표 등)은 사용하지 마.
    뉴스 기사:
    {articles_text}
    """
    ...
    if 'analysis_result' in st.session_state:
        ...
        st.markdown(f'<div class="gemini-summary">{st.session_state.analysis_result}</div>', unsafe_allow_html=True)
