...
if search_query:
    filtered_df = filtered_df[(filtered_df["title"].str.contains(search_query, case=False, na=False)) | (filtered_df.get("summary", pd.Series(dtype='object')).str.contains(search_query, case=False, na=False))]
    if len(filtered_df) == 0:
        ...
        with st.expander(f"📚 {category} ({(filtered_df['category'] == category).sum()}건)", expanded=True):
            ...
            for _, row in cat_df.iterrows():
                ...
                if pd.notna(row.get("url")) and row["url"] != "":
                    ...
                    # expander를 항상 펼친 상태로 변경하고, 높이를 300px로 줄였습니다.
