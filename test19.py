...
with filter_col2:
    ...
    # 데이터의 min/max 날짜를 벗어나지 않도록 설정
    min_data_date = df["date"].min().date()
    max_data_date = df["date"].max().date()
    
    # 오늘 날짜가 데이터 날짜 범위를 벗어나면, 데이터의 최대 날짜로 설정
    if today > max_data_date:
        initial_date = max_data_date
    elif today < min_data_date:
        initial_date = min_data_date
    else:
        initial_date = today

    start_date, end_date = st.date_input(
        "🗓️ 날짜 범위 선택",
        value=(initial_date, initial_date),
