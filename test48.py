...
with tab1:
    if not filtered_df.empty:
        ...
        category_counts.columns = ['카테고리', '뉴스 개수']
    ...
    if st.session_state.likes or st.session_state.dislikes:
        ...
        if like_dislike_data:
            ...
            for idx in ranking_df['index']:
                ...
                liked_urls.append(df.loc[idx].get('url', '#'))
            
            ranking_df['제목'] = liked_titles
            ranking_df['본문 링크'] = liked_urls
            ranking_df['순위'] = range(1, len(ranking_df) + 1)
            
            st.dataframe(ranking_df[['순위', '제목', '본문 링크']], hide_index=True, use_container_width=True)
        else:
