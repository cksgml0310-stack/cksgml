...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'SK networks 뉴스 분석 보고서', 0, 1, 'C')
        self.ln(10)
    ...
    def chapter_body(self, body):
        ...
        self.multi_cell(0, 5, body.encode('cp949', 'replace').decode('cp949'))
        self.ln()

# PDF 다운로드 함수
@st.cache_data
def create_pdf(analysis_title, analysis_result, report_content):
    pdf = PDF()
    # 폰트 등록 (한글 지원)
    pdf.add_font('MalgunGothic', '', 'MalgunGothic.ttf', uni=True)
    pdf.add_font('MalgunGothic', 'B', 'MalgunGothicBold.ttf', uni=True)
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.add_page()
    
    # 통합 인사이트 섹션
    pdf.chapter_title(analysis_title)
    pdf.chapter_body(analysis_result)

    # 보고서 섹션
    pdf.chapter_title("생성된 보고서")
    ...
    if filtered_df.empty:
        ...
        if 'analysis_result' in st.session_state and not st.session_state.get('filters_changed', False):
            ...
            if 'generated_report' in st.session_state:
                ...
                # PDF 다운로드 버튼
                if st.button("📥 PDF 다운로드", key="download_pdf_button"):
                    with st.spinner("PDF를 생성하는 중..."):
                        # FPDF가 한글 폰트를 로드할 수 있도록 .ttf 파일을 포함해야 함
                        # 이 예제에서는 시스템에 설치된 폰트를 가정
                        ...
                        st.download_button(
                            label="PDF 다운로드",
                            data=pdf_output,
                            file_name=f"SK_networks_뉴스_보고서_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
